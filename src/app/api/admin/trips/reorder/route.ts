...
/**
 * PATCH /api/admin/trips/reorder
 *
 * Corpo: `{ ids: [12, 9, 30] }` — o manifesto COMPLETO de roteiros, na ordem
...
export const PATCH = rota(async (request: Request) => {
  ...
  return ok(
    await reordenarCatalogo('trips', ids, { userId: usuario.userId, ip: ipDaRequest(request) }),
  )
})
